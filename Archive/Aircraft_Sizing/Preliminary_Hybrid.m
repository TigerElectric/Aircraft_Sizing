...
LDC         = 15        ; % L/D at cruise conditions (based on AoA and CL)
C_D0        = 0.03      ; % assumed (at cruise): ESTIMATE then ITERAT
LDL         = 17        ; % L/D at loiter conditions (based on AoA and CL)
Cl_stall    = 0.75      ; % Probably at 15 deg, need to find a source, or use airfoil NACA 63-415
...
t_emergency_guess = 60/60; % Guess in [hr] of emergency range time

% DERIVED PARAMETERS (Shouldn't Change Much)
e = 1.78*(1-0.045*AR^(0.68)) - 0.64;
k = 1/(pi*AR*e); % induced drag correction factor, 

Clmax_land  = Cl_stall+1.35; % http://www.fzt.haw-hamburg.de/pers/Scholz/HOOU/AircraftDesign_5_PreliminarySizing.pdf
...
[HPW_takeoff_ws, HPW_climb_ws, HPW_cruise_ws, HPW_turning_max_ws, ...
    ...
    C_D0, g, carpet_x_lim, carpet_y_lim, n_max, n_min);
...
V_loiter        = sqrt(2*W_S*g/rho_air_ci * sqrt(k / (3*C_D0 )))/1.68781;
...
t_emergency_guess, passengers, crew,loiter_dur, SHOW);
...
Emergency_Dist  = t_EM/60*V_cruise;
S               = W_TO / W_S;
...
fus_param_names = {'Length','Max Diameter','Recommended Diameter'...
    ,'Volume Fuselage','Wet Area','Angled Length', 'Angled Starts'};
